...
#  titleLabelWidget.config(text = "サウンドチェック")
  subprocess.call('aplay /home/pi/bezelie/pi/Front_Center.wav', shell=True)
...
#  titleLabelWidget.config(text = "せりふデータを編集します")
  subprocess.call('sudo leafpad /home/pi/bezelie/pi/bezeTalk.csv', shell=True)
...
def configFunction():
  ...
  if configWindow is None or not configWindow.winfo_exists():
    ...
    global originalData
    originalData = []
    with open(csvFile, 'r') as f:  # Open csv file to read
...
def doneConfigFunction():
  ...
  with open(csvFile, 'w') as f:  # opening the file to overwrite
